...
Depending on your application's focus, you'll need a subset of these keys bound as secrets using `npx wrangler secret put <SECRET_NAME>`. Note that `DB` and `ASSETS` are resource bindings configured directly in `wrangler.jsonc`, not secrets.
