...
- **Doc Detection:** On load, fetches `GET /api/docs/:google_doc_id/project`.
  - _If Not Found:_ Displays a welcome banner. Shows "Create New Project" (prompts for name, calls `POST /api/projects`) and "Link to Existing Project" (calls `GET /api/projects`, user selects, calls `POST /api/docs/link`).
  - _If Found:_ Displays project details, task queues, and active Jules sessions.
